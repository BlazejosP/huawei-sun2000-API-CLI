...
echo -e "conecting to \e[4mhttps://eu5.fusionsolar.huawei.com/kiosk/\e[24m"
...
echo "Existing devices in this installation:"
	...
	if [[ $hasInverter == true ]];
		...
		count_inverted_KW=0
			...
			for s in 1; do 
				inverted_KW_array+=( $(echo ''$getPowers''  | jq '.data.inverterPowers[]' | grep -o '[[:digit:]].*[[:digit:]]') )
				(( count_inverted_KW++ ))
			done
...
if [[ $show_data_in_terminal == true ]];
	...
	echo "time of swith on Kiosk Mode: "$time_of_creation
	time_of_update=$(date -d @$updateTime)	
	echo "time of last modification in Kiosk Mode: "$time_of_update
	echo ""
	echo "id: "$id
	...
	echo ""
	echo -e "\e[32mtrees saved: "$forest" trees\e[0m"#!/bin/bash
...
#----------------------
# Here paste URL from your Kiosk mode configuration "Kiosk View Settings" in FussionSolarApp
kiosk_mode_url="https://eu5.fusionsolar.huawei.com/singleKiosk.html?kk=1B8crEoq3j"
